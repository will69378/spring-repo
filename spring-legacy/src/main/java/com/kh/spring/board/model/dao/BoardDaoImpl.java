	...
	public List<Board> selectList(Map<String, Object> paramMap) {
		/*
			특정 페이지의 데이터를 가져오는 방법들(페이징 처리)
				...
				2. OFFSET FETCH를 사용하여 쿼리 조회 (오라클 12이상에서 사용 가능)
				...
				3. RowBounds를 활용한 방식
					...
					- 오라클의 offset fetch 문법과 비슷하며, 어플리케이션으로 가져올 데이터가 수만건
					이상인 경우, 심각한 메모리 낭비 및 성능저하가 발생할 수 있다.
...
// 2. Rownum을 활용한 페이징처리
	...
	public int deleteBoardImg(String deleteList) {
		...
		// ,로 구분되어서 최대까지 4개까지  전달됨 ex) 1,2,3
