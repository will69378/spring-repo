...
@Controller		// ComponentScan에 의해 bean 객체로 등록시키게 하는 속성
...
/*
	...
	Spring DI(Dependency Injection)
		- 의존성 주입
		- 어플리케이션을 구성하는 객체를 개발자가 직접 생성하는게 아닌, 스프링이 생성한 객체
		(bean)를 주입받아서 생성하는 방식
		- new 연산자를 직접 사용하지 않고, Autowird 어노테이션을 통해 주입받는다.
...
/*	
	...
	// 매개변수의 이름과 일치하는 request의 파라미터 값을 추출하여 자동 바인딩
	// 만약 일치하는 파라미터가 없다면 null 값으로 바인딩
/*	public String login(String userId, String userPwd) {
	...
	/*
		...
		@ModelAttribute
			...
			- 요청시 전달한 name 속성과 일치하는 vo클래스 필드의 
			setter 메서드를 호출하여 데이터를 바인딩
			- 매개변수 클래스와 일치하는 클래스의 "기본생성자" 호출 후,
			파라미터의 key 값과 클래스의 필드명이 "일치"하는 경우 setter 호출
	 */
...
/*	public ModelAndView login(Member m, ModelAndView mv, Model model) {
			...
			응답데이터를 담을 수 있는 객체.(Model)
				1) Model
					...
					데이터를 담을 수 있다.
					- 클래스 선언부에 @SessionAttrinbutes를 추가하면 데이터가
					 session scope에 저장된다.
		
				2) ModelAndView
					- ModelAndView에서 Model은 model데이터를 의미한다.
					- VIEW는 이동할 페이지에 대한 정보를 담고 있는 개체를 의미한다.
...
//			return "common/errorPage";
	 ...
	 * @ResponseBody
	 * 
	 * @GetMapping("/selectOne") public Member selectOne(String userId) { Member
	 * searchMember = mService.selectOne(userId);
	 * 
	 * // Jackson-databind를 활용하여, VO클래스, 컬렉션 데이터를 자동으로 // JSON으로 반환해줄 수 있다.
	 * 
	 * return searchMember; }
	 */
	...
	/*
		Spring에서 예외처리 방법
			...
			2. 하나의 컨트롤러에서 발생하는 예외들을 모아서 처리하는 방법
				- 컨트롤러에 예외처리 메서드를 1개 추가한 후,
				 @ExceptionHandler 어노테이션 추가. -> 2순위로 적용	
			3. 어플리케이션 전역에서 발생하는 예외를 모아서 처리하는 방법
				- 새로운 클래스 작성 후 ControllerAdvice 어노테이션 추가 -> 3순위
	 */
